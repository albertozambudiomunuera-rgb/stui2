  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-800">
        ...
        <h3 className="text-xs font-black uppercase tracking-wider text-slate-600 mb-4">Síntomas (preguntas 1–5)</h3>
      ...
      {comp && (
        <>
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-600 mb-4">Indique cuánto le molesta lo siguiente (0=nada, 5=muchísimo).</p>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {[0, 1, 2, 3, 4, 5].map((v) => (
                      <button key={v} onClick={() => actions.updateOABQoL(qi, v)}
                        className={`w-11 min-h-[48px] rounded-xl text-base font-black border-2 transition-all ${
                          data.oab.qol[qi] === v
                            ? 'bg-slate-700 dark:bg-slate-600 border-slate-700 text-white'
                            : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 hover:border-slate-400'
                        }`}>{v}</button>
                    ))}
                    <span className="text-xs text-slate-600 self-center ml-1">0=nada · 5=mucho</span>
                  </div>
                </div>
              ))}
            </div>
